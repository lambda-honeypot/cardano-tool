...
import (
	...
	mock_cli "github.com/lambda-honeypot/cardano-tool/cardano-cli/pkg/mocks/cli"
...
var _ = Describe("CreateUTXOFromAddress", func() {
	var (
		...
		mockRunner *mock_cli.MockRunner
		fundSender FundSender
	)
	BeforeEach(func() {
		ctrl = gomock.NewController(GinkgoT())
		mockRunner = mock_cli.NewMockRunner(ctrl)
...
`
		mockRunner.EXPECT().ExecuteCommand(gomock.Any()).Return([]byte(exampleTXs), nil)
