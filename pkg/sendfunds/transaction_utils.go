...
func buildRawTransactionArgs(utxo *FullUTXO, sourceAddress, outFile string, currentSlot, txOutAdaAmount, minFee int, unsortedPaymentDetails map[string]PaymentDetails, txTokenOutAmounts []TokenDetails, withdrawalString string) []string {
	...
	sortedKeys := make([]string, 0, len(unsortedPaymentDetails))
	for k := range unsortedPaymentDetails {
	...
	for _, addr := range sortedKeys {
		paymentDetails := unsortedPaymentDetails[addr]
	...
	if withdrawalString != "" {
		rawTxArgs = append(rawTxArgs, "--withdrawal")
		rawTxArgs = append(rawTxArgs, withdrawalString)
	}
...
func (fs *FundSender) calculateMinFeeArgs(paramsFile, tempFile, witnessCount string, transactionInCount, transactionOutCount int) []string {
	rawTxArgs := []string{"transaction", "calculate-min-fee", "--tx-body-file", tempFile, "--tx-in-count",
		fmt.Sprint(transactionInCount), "--tx-out-count", fmt.Sprint(transactionOutCount), fs.network, fs.magic, "--witness-count", witnessCount,
